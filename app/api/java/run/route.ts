...
export async function POST(req: Request) {
  ...
  // Replace the public class name so javac accepts it
  ...
  try {
    ...
    // Step 2: run with memory/stack cap
    ...
    const stderr = isTimeout ? `Execution timed out after ${TIMEOUT_MS}ms` : (e.stderr ?? e.message ?? "Unknown error")
    const durationMs = Date.now() - start
